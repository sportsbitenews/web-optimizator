 ...
 * Outputs page to optimize images
 *
 **/
?><noscript><?php
...
?></ul>
<h1 class="wssA wssA5"><?php
	echo _WEBO_TOOLS_IMAGE;
?></h1>
<p class="wssI"><?php
	echo _WEBO_IMAGE_INSTALLED;
?></p>
<p class="wssI"><?php
	echo _WEBO_IMAGE_INSTALLED2;
?></p>
<form action="#wss_image" class="wssC wssC4" method="post" enctype="multipart/form-data"><?php
	if ($submit) {
	?><p class="wssI">
		<a href="#wss_image" class="wssJ7" onclick="_.g(1)"><?php
		echo _WEBO_GZIP_COMPRESS;
		?><span class="wssJ6"></span></a>
	</p><?php
	}
	?><p class="wssI">
		<a href="#wss_image" class="wssJ5" onclick="_('.wssI3')[0].style.display='block';_('.wssC4')[0].onsubmit({target:_('.wssC4')[0]})"><?php
	echo _WEBO_GZIP_FIND;
		?><span class="wssJ6"></span></a>
	</p><dl class="wssD">
		<dd class="wssD1">
			<label for="wss_directory" class="wssE"><?php
	echo _WEBO_GZIP_DIRECTORY;
			?>:</label>
		</dt>
		<dd class="wssD2">
			<input class="wssF" name="wss_directory" id="wss_directory" value="<?php
	...
	echo _WEBO_GZIP_ENTERDIRECTORY;
		...
		<dl class="wssD5">
			<label for="wss_recursive"><?php
	echo _WEBO_GZIP_RECURSIVE;;
			?></label>
		</dl>
		<dd class="wssD6 wssD7">
			<input type="checkbox" name="wss_recursive" id="wss_recursive" title="<?php
	echo _WEBO_GZIP_ENTERRECURSIVE;
			?>"<?php
	echo !$submit || $recursive ? ' checked="checked"' : '';
			?>/><input type="hidden" name="wss_Submit" id="wss_Submit" value="1"/>
		</dd>
	</dl>
<p class="wssA0 wssI3"><?php
	echo _WEBO_GZIP_WAIT;
?>...</p><?php
	if (count($results)) {
?><table class="wssT wssT0">
	<col width="67%"/><col width="10%"/><col width="18%"/><col width="5%"/>
	<thead class="wssT5">
		<tr class="wssT6">
			<th class="wssT7"><?php
		...
		echo _WEBO_GZIP_MTIME;
			?></th>
			<th class="wssT7">
				<input type="checkbox" checked="checked" onclick="_.z()"/>
			</th>
		</tr>
	</thead>
	<tbody><?php
	...
	?></tbody>
</table><table class="wssT wssT0 wssA0">
	<col width="67%"/><col width="15%"/><col width="17%"/>
	<thead class="wssT5">
		<tr class="wssT6">
			<th class="wssT7"><?php
		echo _WEBO_GZIP_RELATIVE;
	...
	<tfoot class="wssT1">
		...
		echo _WEBO_GZIP_INITIAL_TOTAL;
			?>:</th>
			<th class="wssT9" id="wss_total1"></th>
		</tr>
		<tr class="wssT8">
			<th class="wssT9" colspan="2"><?php
		echo _WEBO_GZIP_FINAL_TOTAL;
			?>:</th>
			<th class="wssT9" id="wss_total2"></th>
		</tr>
		<tr class="wssT8">
			<th class="wssT9" colspan="2"><?php
		echo _WEBO_GZIP_SAVINGS;
			?>:</th>
			<th class="wssT9" id="wss_total3"></th>
		</tr>
	</tfoot>
	<tbody></tbody>
</table>
