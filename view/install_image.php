...
?></span></a></li></ul><h1 class="wssA wssA5"><?php
	echo _WEBO_TOOLS_IMAGE;
?></h1><p class="wssI"><?php
	echo _WEBO_IMAGE_INSTALLED;
?></p><p class="wssI"><?php
	echo _WEBO_IMAGE_INSTALLED2;
?></p><form action="#wss_image" class="wssC wssC4" method="post" enctype="multipart/form-data"><?php
	if ($submit) {
?><p class="wssI"><a href="#wss_image" class="wssJ7" onclick="yass.g(1)"><?php
		echo _WEBO_GZIP_COMPRESS;
?><span class="wssJ6"></span></a></p><?php
	}
?><p class="wssI"><a href="#wss_image" class="wssJ5" onclick="yass('.wssI3')[0].style.display='block';var a=yass('.wssC4')[0];if(yass.b.ie){yass[a.name]({target:a})}else{a.onsubmit({target:a})}"><?php
