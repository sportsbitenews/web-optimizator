...
/* are we operating with multiple hosts */
						...
						(!count($ignore_list) || (!in_array(str_replace($old_src_param, '', $img), $ignore_list) && (empty($ignore_list[0]) || strpos($old_src, $ignore_list[0]) === false)))) {
...
/* add info about client side load speed */
			if ($this->debug_mode) {
				$this->content = preg_replace("@(<head([^e][^>]*>|>))@is", "$1" .
					"<script type=\"text/javascript\">//<![CDATA[\n" .
